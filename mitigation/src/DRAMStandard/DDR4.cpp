...
DDR4::DDR4(const YAML::Node& config): config(config)
{
  if (!config["spec"])
...
void DDR4::init_org(const YAML::Node& org_config)
{
...
void DDR4::init_speed(const YAML::Node& speed_config)
{
...
void DDR4::init_prereq()
{
  ...
  // ACT
  prereq[int(Level::Bank)][int(Command::ACT)] = 
  [] (DRAM<DDR4>* node, Command cmd, int id) 
  {
    switch (int(node->state)) 
    {
      case int(State::ClosedPending):     return Command::ACT;
      case int(State::Closed):            return Command::MAX;
      case int(State::Opened):            return Command::PRE;
      default: assert(false && "[PREQ] Invalid bank state for an ACT command!"); return Command::MAX;
    }
  };
    ...
    switch (int(node->state)) 
    {
      ...
      case int(State::Opened):
        if (node->row_state.find(id) != node->row_state.end())
          ...
          return false;
      case int(State::ClosedPending): return false;
...
void DDR4::init_lambda()
{
  ...
  lambda[int(Level::Rank)][int(Command::PDX)] = [] (DRAM<DDR4>* node, int id) { node->state = State::PowerUp; };
  lambda[int(Level::Rank)][int(Command::SRE)] = [] (DRAM<DDR4>* node, int id) { node->state = State::SelfRefresh; };
  lambda[int(Level::Rank)][int(Command::SRX)] = [] (DRAM<DDR4>* node, int id) { node->state = State::PowerUp; };
}


#if defined(RAMULATOR_POWER)
void DDR4::init_power_parameters(const YAML::Node& power_config)
{
...
void DDR4::init_timing_table()
{
  ...
  t[int(Command::WR)]. push_back({Command::WR,  1, TS(nCCDL)});
  t[int(Command::WR)]. push_back({Command::WRA, 1, TS(nCCDL)});
  t[int(Command::WRA)].push_back({Command::WR,  1, TS(nCCDL)});
  t[int(Command::WRA)].push_back({Command::WRA, 1, TS(nCCDL)});
...
uint64_t DDR4::get_nCK_JEDEC_int(float t_ns, int tCK_ps)
{
